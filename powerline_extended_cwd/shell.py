...
class ShellCwdSegment(CwdSegment):
    ...
    def substitute(self, cwd, path, replacement, color):
        cwd = cwd[len(path) + 1:]
        if cwd:
            return [
                self.xterm_header_segment('{}/{}'.format(replacement, cwd)),
                self.regular_segment(replacement, color),
                self.regular_segment(cwd),
            ]

        return [ self.xterm_header_segment(replacement), self.regular_segment(replacement, color) ]
    ...
    def __call__(self, pl, segment_info, shrink=[], **kwargs):
        ...
        if not home:
            return [self.xterm_header_segment(cwd), self.regular_segment(cwd)]

        # check if path is in the config
        for item in shrink:
            path = item[0].replace('~', home)
            if cwd.startswith(path):
                return self.substitute(cwd, path, item[1], 'cwd:split')

        # finally the non-configured home
        if cwd.startswith(home):
            return self.substitute(cwd, home, '~', 'cwd:home')
